...
*/
function deleteOne(str, eliminaPrimocar) {
  if (eliminaPrimocar == true) {
    return str.slice(1);
  } else {
...
console.log(isThisAnEmail("lorenzo@epicode.com"));
console.log(isThisAnEmail("lorenzo-epicode-com")); 
...
function isTodayMyBirthday() {
  ...
  let compleannoGiorno = 22;
  let compleannoMese = 8;
...
console.log(deleteProp(studente, "eta"));
...
function sumAllTheYears() {
  ...
  for (var i = 0; i < movies.length; i++) { 
    totSom = totSom + parseInt(movies[i].Year);
  }
  return totSom;
}
...
*/
function searchByTitle(cercaStringa) {
  let filmTrovato = [];

  for (let i = 0; i < movies.length; i++) {
    if (movies[i].Title.includes(cercaStringa)) {
...
function searchAndDivide(searchString) {
  ...
  let unmatch = [];
  for (let i = 0; i < movies.length; i++) {
    if (movies[i].Title.indexOf(searchString) !== -1) {
      match.push(movies[i]);
    } else {
...
let mioContainer = selezionaContainer();
console.log(mioContainer);
...
*/
function aggiungiRossoAiLink() {
  let tuttiILink = document.getElementsByTagName("a");
  for (let i = 0; i < tuttiILink.length; i++) {  
    tuttiILink[i].style.backgroundColor = "red";
  }
}
...
/* ex 17 */
let risultato = searchByTitle('Avengers');
console.log(risultato);

/* ex 18 */
let ris = searchAndDivide("Avengers");
console.log(ris.match);
console.log(ris.unmatch); 
